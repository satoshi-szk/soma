...
@pytest.mark.parametrize("mode", ["librosa", "native"])
def test_time_stretch_preserves_pitch_and_changes_length(mode: str) -> None:
    sample_rate = 48000
    duration_sec = 1.0
    ...
    slow = time_stretch_pitch_preserving(source, speed_ratio=0.5, sample_rate=sample_rate, mode=mode)
    fast = time_stretch_pitch_preserving(source, speed_ratio=2.0, sample_rate=sample_rate, mode=mode)
